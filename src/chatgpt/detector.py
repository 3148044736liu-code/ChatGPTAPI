...
def normalize_assistant_text(text: str | None) -> str:
    ...
    # ChatGPT can render product-feedback prompts inside the same turn
    # container/copy payload. They are page chrome, not model output.
    cleaned = re.sub(
        r"(?:\r?\n)+\s*Do you like this personality\?\s*$",
        "",
        cleaned,
        flags=re.IGNORECASE,
    ).strip()
            ...
            for (let idx = turns.length - 1; idx >= 0; idx--) {
                ...
                const hasStopButton = Boolean(
                    document.querySelector(
                        'button[data-testid="stop-button"], button[aria-label="Stop answering"], button[aria-label="Stop generating"]'
                    )
                );
...
async def wait_for_response_complete(
    ...
    if completed == "image":
        ...
        return True
    if completed == "stall":
        log.info("Response complete via stall detection — clicking stop to release page")
        await _click_stop_best_effort(page)
        return True

    log.info("Copy/image completion not detected, trying stop-button strategy...")
    try:
        result = await _wait_via_stop_button(page, min(timeout, 45000))
        if result:
            return True
    except Exception as e:
...
async def _wait_for_copy_button_or_image(
    ...
    while elapsed * 1000 < timeout_ms:
        ...
        # Stall detection: finished-looking text that never changes while the
        # stop button stays visible means the backend stream hung. Bail out
        # early instead of burning the whole timeout.
        if is_new_turn and snapshot.get("hasStopButton") and not snapshot.get("hasImage"):
            text = snapshot.get("text") or ""
            if text and not is_incomplete_response_text(text):
                if text == stall_text:
                    if elapsed - stall_since >= STALL_SECONDS:
                        log.warning(
                            f"Stalled generation detected: turn text stable for "
                            f"{int(elapsed - stall_since)}s while stop button is "
                            f"still visible"
                        )
                        return "stall"
                else:
        ...
        if not first_snapshot_logged and elapsed >= 2:
            ...
            turn_chars = len(snapshot.get("text") or "")
            ...
            log.debug(
                ...
                f"text_chars={turn_chars}"
            )
            log.debug("Assistant turn count=%s", len(all_turns))
...
async def _click_stop_best_effort(page: Page) -> None:
    """Click the composer stop button if present; never raises."""
    try:
        for selector in Selectors.STOP_BUTTON:
            btn = await page.query_selector(selector)
            if btn and await btn.is_visible():
                await btn.click()
                log.info(f"Clicked stop button via: {selector}")
                await asyncio.sleep(1.0)
                return
    except Exception as e:
        log.debug(f"Could not click stop button: {e}")
    ...
    try:
        ...
        if isinstance(click_result, dict) and click_result.get("clicked"):
            ...
            if content and content.strip() and content.strip() != str(pre_clipboard).strip():
                content = normalize_assistant_text(content)
                log.info(
    ...
    text = await page.evaluate(
        ...
        (previousSignature) => {
            ...
            for (let idx = turns.length - 1; idx >= 0; idx--) {
                ...
                return ((content || turn).innerText || '').trim();
            }
