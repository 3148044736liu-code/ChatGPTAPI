...
class ChatCompletionRequest(BaseModel):
    ...
    # 非 OpenAI 标准字段（本网关扩展）：
    # 传入一个已通过 POST /v1/sessions 创建的会话 id，本请求就会走
    # 单 Worker 队列执行，并获得用户隔离、消息持久化与全局串行保障。
    # 省略时默认仍走原单页逻辑（除非服务端开启 OPENAI_USE_SESSION_POOL）。
...
class ChatCompletionResponse(BaseModel):
    ...
    # 非 OpenAI 标准字段（本网关扩展）：本次请求实际使用的内部会话 id。
    # 未走会话池时为 null。OpenAI SDK 会自动忽略该字段。
    session_id: Optional[str] = None
